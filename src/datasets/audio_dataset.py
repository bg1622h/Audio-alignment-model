...
class MusicDataset(Dataset):
    ...
    def __init__(self, **kwargs): 
        ...
        waveform_cqt = np.abs(waveform_cqt) #column_count = floor(len(waveform)/hop_size) + 1
        midi_data = midi_processing(self.midi_file, self.hop_size, sr, waveform_cqt.shape[1]) #sec_per_column = (hop_length)/sr
        ...
        for i in range(total_segments):
            ...
            if (audio_segment.size(1) < segment_length):
                ...
                notes_segment = torch.cat((notes_segment, torch.zeros(segment_length - notes_segment.size(0), notes_segment.size(1))), dim = 0)
...
class AudioDataset(BaseDataset):
    ...
    """
    def __init__(self, audio_dir, midi_dir, hop_size, frame_size, dataset_size = None, transform = None, new_sr = None):
