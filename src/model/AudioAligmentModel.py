...
class AudioAligmentModel(nn.Module):
    def __init__(
        ...
        self.conv_layer = nn.Conv1d(
            input_dim, d_model, kernel_size=3, stride=1, padding=1
        )
        ...
        self.conformer = nn.ModuleList(
            [
                ConformerBlock(
                    d_model, nhead, ffn_expansion_factor, kernel_size, dropout
                )
                for i in range(num_blocks)
            ]
        )
        self.final_block1 = ConformerBlock(
            d_model, nhead, ffn_expansion_factor, kernel_size, dropout
        )
        self.final_block2 = ConformerBlock(
            d_model, nhead, ffn_expansion_factor, kernel_size, dropout
        )
